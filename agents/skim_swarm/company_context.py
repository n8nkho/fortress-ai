...
from utils.skim_swarm_config import normalize_symbol, swarm_data_dir

logger = logging.getLogger("skim_swarm.company_context")
...
def context_score_adjustment(ctx: dict[str, Any], features: dict[str, Any]) -> float:
    ...
    if beta is not None:
        try:
            ...
            pass
    if ctx.get("type") == "etf":
        adj += 0.01 * _f(features.get("residual_vs_spy"))
    sector = str(ctx.get("sector") or "")
    if "Technology" in sector or "Semiconductor" in str(ctx.get("industry") or ""):
        adj += 0.04 * _f(features.get("semi_lead_vs_soxx"))
    if ctx.get("symbol") in ("V", "MA") and _f(features.get("vix_last")) > 22:
        adj -= 0.05
