"""Single-symbol infra swarm worker."""
...
from agents.infra_swarm.coordinator import EntrySlotGuard, layer_entry_blocked
from agents.infra_swarm.act import act, cooldown_until_seconds
from agents.infra_swarm.company_context import format_context_blurb
from agents.infra_swarm.features import build_symbol_features
from agents.infra_swarm.intraday_si import log_shadow_decision, shadow_target_mult
from agents.infra_swarm.signal import decide
from agents.infra_swarm.state import load_symbol_state, save_symbol_state
from agents.infra_swarm.symbol_learning import (
...
import agents.infra_swarm.symbol_learning as sym_learn
from utils.infra_swarm_config import shadow_lane_enabled
...
def run_symbol_cycle(
    ...
    *,
    ...
    if action in ("enter_long", "enter_short"):
        ...
        layer_blocked, layer_reason = layer_entry_blocked(sym, positions, side=side)
        if layer_blocked:
            act_result = {
                "action": action,
                "executed": False,
                "detail": layer_reason,
                "block_reason": layer_reason,
            }
        elif entry_guard is not None and not entry_guard.try_reserve():
            act_result = {
                "action": action,
                "executed": False,
    ...
    return {
        ...
        "features": {
            k: features.get(k)
            for k in (
                "last", "r1m", "r5m", "residual_vs_layer", "propagation_lag_vs_l1",
                "unrealized_usd", "side", "layer",
            )
        },
        "decision": decision,
        "act": act_result,
