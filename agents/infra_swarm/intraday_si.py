...
def merge_overlay_into_params(params: dict[str, Any], learned: dict[str, Any]) -> dict[str, float]:
    ...
    try:
        from utils.infra_swarm_config import runtime_overrides as _ro
...
def _block_reason_key(raw: str | None) -> str:
    ...
    if s.startswith("swarm_halted"):
        return "swarm_halted"
    if "infra_target_hit" in s or s.startswith("infra_target"):
        return "target_hit"
...
def log_shadow_decision(symbol: str, record: dict[str, Any]) -> None:
    if not shadow_lane_enabled():
        return
    from agents.infra_swarm.symbol_learning import experience_path
