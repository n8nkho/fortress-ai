#!/usr/bin/env python3
"""
Fortress AI Infra Swarm — AI infrastructure layer propagation, no LLM, always-on.

Stack Residual Propagation (SRP): per-symbol recursive agents on an adaptive universe.
"""
...
from agents.infra_swarm.company_context import load_all_contexts
from agents.infra_swarm.coordinator import (
    EntrySlotGuard,
    apply_daily_pnl,
    count_open,
    max_open_ok,
...
from agents.infra_swarm.eod import describe_eod_phase
from agents.infra_swarm.features import build_shared_context, _fetch_bars
from agents.infra_swarm.observe import observe_account
from agents.infra_swarm.session_reconcile import reconcile_session_on_boot
from agents.infra_swarm.symbol_learning import sync_adaptive_state_on_boot
from agents.infra_swarm.state import load_swarm_state, save_swarm_state
from agents.infra_swarm.worker import run_symbol_cycle
...
def run_loop(iterations: int | None = None) -> None:
    ...
    while iterations is None or n < iterations:
        ...
        if n > 0 and n % 20 == 0:
            universe_refresh = refresh_adaptive_universe()
            syms = universe()
...
def main() -> None:
    ...
    ap = argparse.ArgumentParser(description="Fortress AI infra swarm")
